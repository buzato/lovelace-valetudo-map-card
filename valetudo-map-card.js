class ValetudoMapCard extends HTMLElement {
    ...
    shouldDrawControls(state) {
        return (
            ...
            this.lastUpdatedControls !== state.last_updated
        )
    }
    ...
    calculateColor(container, ...colors) {
        for (let color of colors) {
            ...
            if (color.startsWith('--')) {
                let possibleColor =
                    getComputedStyle(container).getPropertyValue(color)
    ...
    isOutsideBounds(x, y, drawnMapCanvas, config) {
        return (
            x < this._config.crop.left ||
            x > drawnMapCanvas.width ||
            y < config.crop.top ||
            y > drawnMapCanvas.height
        )
    }
    ...
    drawMap(attributes, mapHeight, mapWidth, boundingBox) {
        ...
        const floorColor = this.calculateColor(
            homeAssistant,
            this._config.floor_color,
            '--valetudo-map-floor-color',
            '--secondary-background-color'
        )
        const wallColor = this.calculateColor(
            homeAssistant,
            this._config.wall_color,
            '--valetudo-map-wall-color',
            '--accent-color'
        )
        const currentlyCleanedZoneColor = this.calculateColor(
            homeAssistant,
            this._config.currently_cleaned_zone_color,
            '--valetudo-currently_cleaned_zone_color',
            '--secondary-text-color'
        )
        const noGoAreaColor = this.calculateColor(
            homeAssistant,
            this._config.no_go_area_color,
            '--valetudo-no-go-area-color',
            '--accent-color'
        )
        const noMopAreaColor = this.calculateColor(
            homeAssistant,
            this._config.no_mop_area_color,
            '--valetudo-no-mop-area-color',
            '--secondary-text-color'
        )
        const virtualWallColor = this.calculateColor(
            homeAssistant,
            this._config.virtual_wall_color,
            '--valetudo-virtual-wall-color',
            '--accent-color'
        )
        const pathColor = this.calculateColor(
            homeAssistant,
            this._config.path_color,
            '--valetudo-map-path-color',
            '--primary-text-color'
        )
        const chargerColor = this.calculateColor(
            homeAssistant,
            this._config.dock_color,
            'green'
        )
        const vacuumColor = this.calculateColor(
            homeAssistant,
            this._config.vacuum_color,
            '--primary-text-color'
        )
        const gotoTargetColor = this.calculateColor(
            homeAssistant,
            this._config.goto_target_color,
            'blue'
        )
        ...
        if (this._config.show_dock && chargerInfo) {
            ...
            chargerHTML.style.left = `${
                Math.floor(chargerInfo[0] / widthScale) -
                objectLeftOffset -
                mapLeftOffset -
                12 * this._config.icon_scale
            }px`
            chargerHTML.style.top = `${
                Math.floor(chargerInfo[1] / heightScale) -
                objectTopOffset -
                mapTopOffset -
                12 * this._config.icon_scale
            }px`
        ...
        if (this._config.show_vacuum && robotInfo) {
            ...
            vacuumHTML.style.left = `${
                Math.floor(robotInfo[0] / widthScale) -
                objectLeftOffset -
                mapLeftOffset -
                12 * this._config.icon_scale
            }px`
            vacuumHTML.style.top = `${
                Math.floor(robotInfo[1] / heightScale) -
                objectTopOffset -
                mapTopOffset -
                12 * this._config.icon_scale
            }px`
        ...
        if (this._config.show_goto_target && goToInfo) {
            ...
            goToTargetHTML.style.left = `${
                Math.floor(goToInfo[0] / widthScale) -
                objectLeftOffset -
                mapLeftOffset -
                12 * this._config.icon_scale
            }px`
            goToTargetHTML.style.top = `${
                Math.floor(goToInfo[1] / heightScale) -
                objectTopOffset -
                mapTopOffset -
                22 * this._config.icon_scale
            }px`
        ...
        if (this._config.show_floor) {
            ...
            if (floorPoints) {
                for (let i = 0; i < floorPoints.length; i += 2) {
                    let x =
                        floorPoints[i] * this._config.map_scale - mapLeftOffset
                    let y =
                        floorPoints[i + 1] * this._config.map_scale -
                        mapTopOffset
                    if (
                        this.isOutsideBounds(x, y, drawnMapCanvas, this._config)
                    )
                        continue
                    mapCtx.fillRect(
                        x,
                        y,
                        this._config.map_scale,
                        this._config.map_scale
                    )
                }
            }
            ...
            for (let item of segmentAreas) {
                mapCtx.strokeStyle =
                    this._config.segment_colors[
                        colorFinder.getColor(item.metaData.segmentId)
                    ]
                mapCtx.lineWidth = 1
                mapCtx.fillStyle =
                    this._config.segment_colors[
                ...
                if (segmentPoints) {
                    for (let i = 0; i < segmentPoints.length; i += 2) {
                        let x =
                            segmentPoints[i] * this._config.map_scale -
                            mapLeftOffset
                        let y =
                            segmentPoints[i + 1] * this._config.map_scale -
                            mapTopOffset
                        if (
        ...
        if (this._config.show_walls) {
            ...
            if (wallPoints) {
                for (let i = 0; i < wallPoints.length; i += 2) {
                    let x =
                        wallPoints[i] * this._config.map_scale - mapLeftOffset
                    let y =
                        wallPoints[i + 1] * this._config.map_scale -
                        mapTopOffset
                    if (
                        this.isOutsideBounds(x, y, drawnMapCanvas, this._config)
                    )
        ...
        if (
            Array.isArray(activeZones) &&
            activeZones.length > 0 &&
            this._config.show_currently_cleaned_zones
        ) {
            ...
            for (let item of activeZones) {
                ...
                for (let i = 0; i < points.length; i += 2) {
                    let x =
                        Math.floor(points[i] / widthScale) -
                        objectLeftOffset -
                        mapLeftOffset
                    let y =
                        Math.floor(points[i + 1] / heightScale) -
                        objectTopOffset -
                        mapTopOffset
                    if (i === 0) {
                        mapCtx.moveTo(x, y)
        ...
        if (
            virtualWallPoints &&
            this._config.show_virtual_walls &&
            this._config.virtual_wall_width > 0
        ) {
            ...
            for (let item of virtualWallPoints) {
                let fromX =
                    Math.floor(item['points'][0] / widthScale) -
                    objectLeftOffset -
                    mapLeftOffset
                let fromY =
                    Math.floor(item['points'][1] / heightScale) -
                    objectTopOffset -
                    mapTopOffset
                let toX =
                    Math.floor(item['points'][2] / widthScale) -
                    objectLeftOffset -
                    mapLeftOffset
                let toY =
                    Math.floor(item['points'][3] / heightScale) -
                    objectTopOffset -
                    mapTopOffset
                if (
                    this.isOutsideBounds(
                    ...
                    continue
                if (
                    this.isOutsideBounds(toX, toY, drawnMapCanvas, this._config)
                )
                    continue
        ...
        if (
            ...
            pathPoints.length > 0 &&
            this._config.show_path &&
            this._config.path_width > 0
        ) {
            for (let item of pathPoints) {
                ...
                for (let i = 0; i < item.points.length; i += 2) {
                    x =
                        Math.floor(item.points[i] / widthScale) -
                        objectLeftOffset -
                        mapLeftOffset
                    y =
                        Math.floor(item.points[i + 1] / heightScale) -
                        objectTopOffset -
                        mapTopOffset
                    if (
                        this.isOutsideBounds(x, y, drawnMapCanvas, this._config)
        ...
        if (
            Array.isArray(predictedPathPoints) &&
            predictedPathPoints.length > 0 &&
            this._config.show_predicted_path &&
            this._config.path_width > 0
        ) {
    ...
    drawControls(infoEntity) {
        ...
        if (infoEntity && infoEntity.state && this._config.show_status) {
            ...
            statusInfo.innerHTML =
                infoEntity.state[0].toUpperCase() +
                infoEntity.state.substring(1)
            this.infoBox.appendChild(statusInfo)
        }

        if (
            infoEntity &&
            infoEntity.attributes &&
            infoEntity.attributes.battery_icon &&
            infoEntity.attributes.battery_level &&
            this._config.show_battery_level
        ) {
        ...
        // Create controls
        if (
            this._config.show_start_button &&
            this.shouldDisplayButton('start', infoEntity.state)
        ) {
            ...
            startButton.addEventListener('click', (event) => {
                this._hass
                    .callService('vacuum', 'start', {
                        entity_id: this.getVacuumEntityName(
                            this._config.vacuum
                        ),
                    })
                    .then()
        ...
        if (
            this._config.show_pause_button &&
            this.shouldDisplayButton('pause', infoEntity.state)
        ) {
        ...
        if (
            this._config.show_stop_button &&
            this.shouldDisplayButton('stop', infoEntity.state)
        ) {
        ...
        if (
            this._config.show_home_button &&
            this.shouldDisplayButton('home', infoEntity.state)
        ) {
            ...
            homeButton.addEventListener('click', (event) => {
                this._hass
                    .callService('vacuum', 'return_to_base', {
                        entity_id: this.getVacuumEntityName(
                            this._config.vacuum
                        ),
                    })
        ...
        for (let i = 0; i < this._config.custom_buttons.length; i++) {
            ...
            if (
                custom_button === Object(custom_button) &&
                custom_button.service &&
                custom_button.service.includes('.')
            ) {
                const customButton = document.createElement('paper-button')
                const customButtonIcon = document.createElement('ha-icon')
                const customButtonRipple =
                    document.createElement('paper-ripple')
                customButtonIcon.icon =
                    custom_button['icon'] || 'mdi:radiobox-blank'
    ...
    setConfig(config) {
        this._config = Object.assign({}, DEFAULT_CARD_CONFIG, config)
        ...
        this.cardContainer.style.background =
            this._config.background_color ?? null
    ...
    set hass(hass) {
        ...
        let mapEntity = this.getMapEntity(this._config.mapEntity)
        ...
        if (vacuumEntity && vacuumEntity.state !== this.lastRobotState) {
            this.pollInterval =
                POLL_INTERVAL_STATE_MAP[vacuumEntity.state] || 10000
        ...
        if (
            mapEntity &&
            mapEntity['state'] !== 'unavailable' &&
            attributes?.['entity_picture']
        ) {
            if (
                new Date().getTime() - this.pollInterval >
                    this.lastMapPoll.getTime() ||
                shouldForcePoll
            ) {
                this.loadImageAndExtractMapData(attributes['entity_picture'])
                    .then((mapData) => {
                        if (mapData !== null) {
            ...
            this.entityWarning1.textContent = `Entity not available: ${this.getMapEntityName(
                this._config.mapEntity
            )}`
    ...
    async loadImageAndExtractMapData(url) {
        if (this.isPollingMap === false) {
            ...
            if (!response.ok) {
                throw new Error(
                    'Got error while fetching image ' +
                        response.status +
                        ' - ' +
                        response.statusText
                )
            ...
            const chunks = extractZtxtPngChunks(
                new Uint8Array(responseData)
            ).filter((c) => c.keyword === 'ValetudoMap')
    ...
    handleDrawing(hass, mapEntity, attributes) {
        ...
        this.mapContainer.style.display =
            !canDrawMap || !this._config.show_map ? 'none' : 'block'
        ...
        if (!canDrawControls) {
            ...
            // Show the warning
            this.entityWarning2.textContent = `Entity not available: ${this.getVacuumEntityName(
                this._config.vacuum
            )}`
      ...
      `
            ...
            if (this.shouldDrawMap() && this._config.show_map) {
                ...
                this.drawMap(attributes, mapHeight, mapWidth, boundingBox)
...
const DEFAULT_CARD_CONFIG = {
    ...
    segment_colors: ['#19A1A1', '#7AC037', '#DF5618', '#F7C841'],
                ...
                function (e, t, i) {
                    ...
                    function H(e) {
                        if (u) {
                            ...
                            for (
                                C(I, e.lens, 0, 288, d, 0, e.work, { bits: 9 }),
                                    t = 0;
                                t < 32;

                            )
                                e.lens[t++] = 5
                            C(D, e.lens, 0, 32, c, 0, e.work, { bits: 5 }),
                                (u = !1)
                        }
                        ;(e.lencode = d),
                            (e.lenbits = 9),
                        ...
                        if (
                            ...
                            t.dictionary &&
                                ('string' == typeof t.dictionary
                                    ? (t.dictionary = h.string2buf(
                                          t.dictionary
                                      ))
                                    : '[object ArrayBuffer]' ===
                                          m.call(t.dictionary) &&
                                      (t.dictionary = new Uint8Array(
                                          t.dictionary
                                      )),
                                t.raw &&
                                    (i = c.inflateSetDictionary(
                                        this.strm,
                                        t.dictionary
                                    )) !== b.Z_OK))
                        )
                                ...
                                ((0 !== s.avail_out &&
                                    ...
                                    ('string' === this.options.to
                                        ? ((a = h.utf8border(
                                          ...
                                          this.onData(o))
                                        : this.onData(
                                              u.shrinkBuf(s.output, s.next_out)
                                          ))),
                                0 === s.avail_in &&
                                    0 === s.avail_out &&
                                    (d = !0)
